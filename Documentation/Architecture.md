...
- **Deterministic tokens (phase 2)**: The zero-access upload screen now ships with a lightweight streaming `.mbox` parser written in JS that extracts header metadata (`from`, `to`, `cc`, `subject`) while the plaintext file is still on the user’s device. Each token type is normalized, deduplicated, capped (2 000 messages / 512 tokens per type), HMAC’d with the same HKDF-derived key, and committed alongside the legacy tag tokens. `/api/v1/mailboxes` exposes matching filters (`fromToken`, `toToken`, `subjectToken`) so a Blazor admin can hash a value locally and ask the API “which encrypted mailboxes contain mail from this sender?” without touching ciphertext.
