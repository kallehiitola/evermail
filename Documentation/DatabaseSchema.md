...
Captures per-tenant customer-managed key (CMK) configuration, supported KMS providers (Evermail-managed/Azure/AWS), and secure key release setup.

```sql
CREATE TABLE TenantEncryptionSettings (
    TenantId UNIQUEIDENTIFIER NOT NULL PRIMARY KEY,
    
    Provider NVARCHAR(50) NOT NULL DEFAULT 'AzureKeyVault', -- EvermailManaged, AzureKeyVault, AwsKms
    
    -- Azure Key Vault fields (legacy + Evermail-managed flow)
    ...
    -- AWS KMS (external provider) metadata
    AwsAccountId NVARCHAR(32) NULL,
    AwsRegion NVARCHAR(32) NULL,
    AwsKmsKeyArn NVARCHAR(2048) NULL,
    AwsIamRoleArn NVARCHAR(2048) NULL,
    AwsExternalId NVARCHAR(128) NULL, -- Generated by Evermail, used in AssumeRole to avoid confused-deputy attacks
    
    ProviderMetadata NVARCHAR(MAX) NULL, -- JSON blob for future providers (GCP KMS, on-prem HSM, etc.)
...
> This table uses a 1:1 relationship with `Tenants` so deleting a tenant automatically removes its encryption metadata. `ProviderMetadata` stores serialized settings for future KMS connectors so the schema does not require another migration for every new provider.
