...
### 2025-11-25 - Confidential worker wiring & ingestion smoke test
- 🔑 Mirrored the Offline BYOK protector into Azure production: stored a 256-bit `OfflineByok--MasterKey` in `evermail-prod-kv`, surfaced it to the worker as the `offline-master` secret, and set `OfflineByok__MasterKey=secretref:offline-master` so Confidential Container Apps can decrypt tenant bundles without leaking plaintext keys.
- 🏗️ Patched `Documentation/Deployment.md` with the exact post-provision commands (`ConnectionStrings__key-vault`, secret wiring) so future operators can reproduce the setup without spelunking container revisions.
- ✉️ Seeded a pilot tenant/mailbox directly in the new Azure SQL instance, uploaded a sample `.mbox` to `mailbox-archives/{tenantId}/{mailboxId}/`, and enqueued the corresponding `mailbox-ingestion` message to prove the remote worker drains real queues.
- 📈 Tailed `evermail-conf-worker` logs to confirm the ingestion lifecycle (`Queued → Processing → Completed`), verified `Mailboxes.ProcessedEmails = 1`, and documented the runbook in `Deployment.md` under “Production smoke test” for future readiness checks.
- 🔁 Updated `Evermail.AppHost/Program.cs` so publish mode now consumes the Key Vault-backed `ConnectionStrings--evermaildb` secret instead of trying to spin up a SQL container in Azure; local dev keeps the Dockerized SQL Server path, and the deployment guide no longer references unused resource-name parameters.

### 2025-11-25 - Deterministic header tokens for zero-access mailboxes
- 📨 Extended the zero-access upload flow so the browser now streams `.mbox/.mbx` headers, extracts normalized `from`/`to`/`cc`/`subject` values, and derives deterministic HMAC tokens alongside the existing manual tags. The new JS helper keeps memory usage bounded (2 000 messages, 512 unique values per type) and cleanly skips unsupported formats with user-facing messaging.
- 🧭 Updated the Blazor upload experience with a “Header indexing” status card that surfaces how many unique senders/subjects were captured, highlights when limits were hit, and lets admins rescan before uploading. CompleteZeroAccess requests now include multiple `tokenSets`, and `ZeroAccessMailboxTokens` deduplicates + caps entries server-side.
- 🔍 Enhanced `/api/v1/mailboxes` so callers can pass `tagToken`, `fromToken`, `toToken`, `ccToken`, and/or `subjectToken` query parameters (all derived client-side) to filter encrypted mailboxes before downloading ciphertext. Documentation (`Security.md`, `Architecture.md`, `API.md`) describes the derivation rules, limits, and the new query surface.
